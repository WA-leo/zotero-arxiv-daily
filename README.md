...
- A machine-readable `review.json` queue is generated for human confirmation.
...
The workflow also accepts these public repository variables. They are applied as runtime overrides, so they take precedence over the corresponding values in `CUSTOM_CONFIG`:
