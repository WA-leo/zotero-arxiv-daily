...
def test_send_email_uses_ssl_on_port_465(config, monkeypatch):
    ...
    class UnexpectedSMTP:
        def __init__(self, *args, **kwargs):
            raise AssertionError("port 465 must use implicit TLS")
...
def test_send_email_retries_transient_disconnect(config, monkeypatch):
    ...
    monkeypatch.setattr("zotero_arxiv_daily.utils.time.sleep", lambda _: None)
    send_email(config, "<html>retry</html>")
